...
export default class FormAnime extends React.Component {
  ...
  comprobar () {
    if (this.state.titulo.length > 0 && this.state.num_cap.length > 0 && this.state.dur_cap.length > 0 && this.state.sinopsis.length > 0 && this.state.op_perso.length > 0 && this.state.generos.length > 0 && this.state.productora.length > 0 ) {
      return true;
    }
    else {
      return false;
